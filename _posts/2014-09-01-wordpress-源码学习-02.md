...
if ( ( 0 === validate\_file( $locale ) ) && is\_readable( $locale\_file ) )
...
if ( ! defined( 'WP\_INSTALLING' ) || 'wp-activate.php' === $pagenow ) {
	if ( TEMPLATEPATH !== STYLESHEETPATH && file\_exists( STYLESHEETPATH . '/functions.php' ) )
...
function wp\_fix\_server\_vars() {
	...
	if ( empty( $\_SERVER['REQUEST\_URI'] ) || ( php\_sapi\_name() != 'cgi-fcgi' && preg\_match( '/^Microsoft-IIS//', $\_SERVER['SERVER\_SOFTWARE'] ) ) ) {
		...
		// IIS Isapi\_Rewrite
			...
			if ( isset( $\_SERVER['PATH\_INFO'] ) ) {
				if ( $\_SERVER['PATH\_INFO'] == $\_SERVER['SCRIPT\_NAME'] )
	...
	if ( isset( $\_SERVER['SCRIPT\_FILENAME'] ) && ( strpos( $\_SERVER['SCRIPT\_FILENAME'], 'php.cgi' ) == strlen( $\_SERVER['SCRIPT\_FILENAME'] ) - 7 ) )
		$\_SERVER['SCRIPT\_FILENAME'] = $\_SERVER['PATH\_TRANSLATED'];

	// Fix for Dreamhost and other PHP as CGI hosts
	if ( strpos( $\_SERVER['SCRIPT\_NAME'], 'php.cgi' ) !== false )
...
function wp\_favicon\_request() {
	if ( '/favicon.ico' == $\_SERVER['REQUEST\_URI'] ) {
...
function wp\_debug\_mode() {
	if ( WP\_DEBUG ) {
		...
		elseif ( null !== WP\_DEBUG\_DISPLAY )
...
function wp\_not\_installed() {
	...
	} elseif ( ! is\_blog\_installed() && false === strpos( $\_SERVER['PHP\_SELF'], 'install.php' ) && !defined( 'WP\_INSTALLING' ) ) {
